...
//93LC46 ÅäÖÃ
void IC_93Ci2cSetSDA(BYTE value)
{
	if(value)
	{
		i2cSetSDA(_HIGH);	//P1_6=1;
	}
	else
	{
		i2cSetSDA(_LOW);	//P1_6=0;
	}

#if 0
if(value)
MonGPIO_Out(A100_DATE_PORT, A100_DATE_PIN, 1);
else
MonGPIO_Out(A100_DATE_PORT, A100_DATE_PIN, 1);
#endif
}
...
void IC_93Ci2cSetSCL(BYTE value)
{
	if(value)
	{
		i2cSetSCL(_HIGH);//P1_7=1;
	}
	else
	{
		i2cSetSCL(_LOW);//P1_7=0;
	}

#if 0
if(value)
MonGPIO_Out(A100_CLK_PORT, A100_CLK_PIN, 1);
else
MonGPIO_Out(A100_CLK_PORT, A100_CLK_PIN, 1);
#endif
}

BYTE IC_93Ci2cSDA_PIN_STATUS(void)
{
	return i2cSDA_PIN_STATUS();//P1_6;
}

BYTE IC_93C_i2c_Send_byte(BYTE ucVal,BYTE count,BYTE ack)
{
	BYTE ucMask = 0x80;
	...
	ucMask>>=(8-count);
	IC_93Ci2cSetSCL(_LOW);
		//	SET_SDA_OUT();
	while (ucMask)
	{
		if (ucVal & ucMask)
		{
			IC_93Ci2cSetSDA(_HIGH);
		}
		else
		{
			IC_93Ci2cSetSDA(_LOW);
		}
		i2c_Delay();
		IC_93Ci2cSetSCL(_HIGH); // clock
		i2c_Delay();
		IC_93Ci2cSetSCL(_LOW);
		i2c_Delay();
	...
	if(ack)
	{
		...
		//IC_93Ci2cSetSDA(_LOW);
		i2c_Delay();
		//IC_93Ci2cSetSCL(_HIGH);
		i2c_Delay();
		SET_SDA_IN();
		if (IC_93Ci2cSDA_PIN_STATUS())
		{
			bAck = _HIGH;
		}
		//no ack
		...
		SET_SDA_OUT();
			i2c_Delay();
			 Printf1("\r\n ucSlaveAdr11111111====%d", (WORD)bAck);
		return (1-bAck);
		#endif
	}
...
BYTE IC_93Ci2c_ReceiveByte(void)
{
    BYTE ucReceive = 0;
    BYTE ucMask = 0x80;
...
BYTE IC_93C_i2c_Send_type(BYTE addr,BYTE cmd)
{
    switch(cmd)
    	{
    	case read:
			return IC_93C_i2c_Send_byte(0x06,3,0);
    	case read_addr:
			return IC_93C_i2c_Send_byte(0x00,7,1);
    	case write:
			return IC_93C_i2c_Send_byte(0x05,3,0);
    	case write_addr:
			return IC_93C_i2c_Send_byte(addr,7,1);
    	case write_en:
			//MonGPIO_Out(A100_CS_PORT, A100_CS_PIN, 1);
			A100_CS_PORT_SET();
			//Delay_1ms(12);
			 IC_93C_i2c_Send_byte(0x04,3,0);
			  IC_93C_i2c_Send_byte(0x60,7,0);
...
void IC_93Ci2cBurstReadBytes( BYTE *pBuf, BYTE ucBufLen)
{
	...
	//	    IC_93Ci2cSetSDA(_LOW);
		...
		// IC_93Ci2cSetSCL(_LOW);
		// i2c_Delay();
        if (IC_93C_i2c_Send_type(0,read) == FALSE)
        {
           // continue;
        }
		if (IC_93C_i2c_Send_type(0,read_addr) == FALSE)
...
void IC_93Ci2cBurstWriteBytes(BYTE addr,BYTE *pBuf, BYTE ucBufLen)
{
    WORD ucDummy; // loop dummy
    ucDummy = 1;
    while (ucDummy--)
...
//	IC_93C_i2c_Send_type(write_erase);
        //A100_CS_PORT_SET();
		IC_93C_i2c_Send_type(0,write_en);
	...
	//	    IC_93Ci2cSetSDA(_LOW);
		...
		// IC_93Ci2cSetSCL(_LOW);
		// i2c_Delay();
		//IC_93C_i2c_Send_type(write_all);
	#if 1
        if (IC_93C_i2c_Send_type(0,write) == FALSE)
        {
           // continue;
        }
		if (IC_93C_i2c_Send_type(addr,write_addr) == FALSE)
            // check non-acknowledge
        {
            //continue;
        }
	   ...
	   IC_93C_i2c_Send_type(0,write_ds);
    } // while
    //i2c_Stop();
}
