...
*/
const WORD FOsdHwDefPaletteBpp1[16] =
{
...
*/
const WORD FOsdSwDefPaletteBpp1[16] =
{
    /*0:Black*/			FOSD_COLOR_VALUE_BLACK,
    /*9:Blue*/			FOSD_COLOR_VALUE_BLUE,
    /*A:Lime*/			FOSD_COLOR_VALUE_LIME,
...
*/
const  WORD FOsdDefPaletteBpp2[4] =
{
...
*/
const  WORD FOsdDefPaletteBpp3[8] =
{
...
*/
const  BYTE FOsdDefPaletteBpp3Alpha[8] = { 1,7,6,4,3,5,2,0 };		//consolas22_16x26

/**
* FOSD 3 BPP default color table
*/
const  WORD FOsdDefPaletteBpp4[16] =
{
...
const 	WORD	FOSDWinBase[] = { FONTWIN1_ST, FONTWIN2_ST, FONTWIN3_ST, FONTWIN4_ST, FONTWIN5_ST, FONTWIN6_ST, FONTWIN7_ST, FONTWIN8_ST };



WORD default_LUT_bpp2[4] 		= { 0x0000,0x001F,0xF800,0xFFFF };
WORD default_LUT_bpp3[8] 		= { 0x0000,0x001F,0x07E0,0x07FF,0xF800,0xF81F,0xFFE0,0xFFFF };
WORD default_LUT_bpp4[16] 		= { 0x0000,0x0010,0x0400,0x0410,0x8000,0x8010,0x8400,0x8410,
                                    0xC618,0x001F,0x07E0,0x07FF,0xF800,0xF81F,0xFFE0,0xFFFF
                              };


WORD graynum_LUT_bpp3[8] 		= { 0xFFFF,0x0000,0xDEDB,0x9492,0x6B6D,0xB5B6,0x4A49,0x2124 };
//const  WORD graynum_LUT_bpp4[16] 	= {	0xD6BA,0x20E3,0xF79E,0x62E8,0xE104,0xA944,0x39A6,0x7BAC,
//										0x51A6,0xC617,0x9CD1,0xB5B5,0x9BC9,0xDD85,0xF643,0xAC87};

#define FONT_ADDR 0xE00000
//                                              loc,      size    W   H   2BPP   3BPP   4BPP   MAX    palette for 2bpp,3bpp,4bpp
const FONT_SPI_INFO_t default_font 		   	= { FONT_ADDR+0x0000, 0x27F9, 12, 18, 0x100, 0x120, 0x15F, 0x17B, default_LUT_bpp2, default_LUT_bpp3, default_LUT_bpp4 };
const FONT_SPI_INFO_t consolas16x26_606C90 	= { FONT_ADDR+0x3000, 0x2080, 16, 26, 0x060, 0x06C, 0x090, 0x0A0, NULL, NULL, NULL };
const FONT_SPI_INFO_t consolas16x26_graynum 	= { FONT_ADDR+0x6000, 0x0618, 16, 26, 0x000, 0x000, 0x01E, 0x01E, NULL, graynum_LUT_bpp3, NULL };
const FONT_SPI_INFO_t kor_font		 		= { FONT_ADDR+0x9000, 0x0A20, 12, 18, 0x000, 0x000, 0x000, 0x060, NULL, NULL, NULL };
const FONT_SPI_INFO_t ram_font		 		= { FONT_ADDR+0xB000, 0x2080, 16, 18, 0x060, 0x06C, 0x090, 0x0A0, NULL, NULL, NULL };

//BKTODO: move default font in this bank.
//it have a BANK issue.
BYTE BPP3_alpha_lut_offset[8];					//need base_lut to use.
const  BYTE BPP3_alpha_value[8]= {0,2,3,4,5,6,7,8};


BYTE FOsdMsgBuff[50];  //for FontOsd Message
//NOTE: If you have more than 40, system will be corrupted.

BYTE DisplayColumns=10;
...
*
* old const  use index 2 for normal foreground	   Now 6bit.(64).
...
* TW8835 needs PCLK divider2
*/
void FOsdSetPaletteColorArray(BYTE index,const WORD *array, BYTE size, BYTE vdelay)
{
    BYTE i;
    BYTE r30c;
...
void FOsdRamSetAddr(WORD OsdRamAddr)
{
    TW8836_WritePage(PAGE3_FOSD );
    FOsdRamSetWriteMode(FOSD_OSDRAM_WRITE_NORMAL);
    FOsdRamSetAddress(OsdRamAddr);
...
//	InitFontRamByNum(FONT_NUM_DEF12X18, 0);	// set consolas & graynum and calculate 3BPP alpha
    ...
    for(winno = 0; winno < 3; winno++)
    {
        FOsdWinInit(winno);

//        FontOsdInfo.win[winno].bpp1_attr = 0x1A;					//BG|FG
		FontOsdInfo.win[winno].bpp1_attr = (bgColor << 4) | fgColor;
...
//		FontOsdInfo.win[0].bpp1_attr = 0x1A;					//BG|FG
    FontOsdInfo.win[FOSD_MSG_WIN].bpp1_attr = (bgColor << 4) | fgColor;
    FOsdWinAlphaPixel(FOSD_MSG_WIN, FOSD_COLOR_IDX_BLANK, 0x0F);		//BG color alpha

    //FOsdWinScreenXY(winno, 0,30*winno);
    FOsdWinScreenXY(FOSD_MSG_WIN, 800,100);//30*winno);
    FOsdWinScreenWH(FOSD_MSG_WIN, columns, 1);		//0x20, 0x10);		//max 512 = 32x16
    FOsdWinZoom(FOSD_MSG_WIN, 0, 0);
    FOsdWinSetOsdRamStartAddr(FOSD_MSG_WIN, 0 );//text_info->osdram
#endif
...
void FOsdWinDisplayInit(void)
{
    ...
    FontOsdInfo.win[FOSD_WIN0].bpp1_attr = (bgColor << 4) | fgColor;
    FOsdWinAlphaPixel(FOSD_WIN0, FOSD_COLOR_IDX_BLANK, 0x0F);		//BG color alpha

    FOsdWinScreenXY(FOSD_WIN0, 800,100);//30*winno);
    FOsdWinScreenWH(FOSD_WIN0, columns, 1);		//0x20, 0x10);		//max 512 = 32x16
    FOsdWinZoom(FOSD_WIN0, 0, 0);
    FOsdWinSetOsdRamStartAddr(FOSD_WIN0, 0 );//text_info->osdram

    FontOsdInfo.win[FOSD_WIN1].bpp1_attr = (bgColor << 4) | fgColor;
    FOsdWinAlphaPixel(FOSD_WIN1, FOSD_COLOR_IDX_BLANK, 0x0F);		//BG color alpha

    FOsdWinScreenXY(FOSD_WIN1, 800,100);//30*winno);
    FOsdWinScreenWH(FOSD_WIN1, columns, 1);		//0x20, 0x10);		//max 512 = 32x16
    FOsdWinZoom(FOSD_WIN1, 0, 0);
    FOsdWinSetOsdRamStartAddr(FOSD_WIN1, columns*FOSD_WIN1);//text_info->osdram

    FontOsdInfo.win[FOSD_WIN2].bpp1_attr = (bgColor << 4) | fgColor;
    FOsdWinAlphaPixel(FOSD_WIN2, FOSD_COLOR_IDX_BLANK, 0x0F);		//BG color alpha

    FOsdWinScreenXY(FOSD_WIN2, 800,100);//30*winno);
    FOsdWinScreenWH(FOSD_WIN2, columns, 1);		//0x20, 0x10);		//max 512 = 32x16
    FOsdWinZoom(FOSD_WIN2, 0, 0);
    FOsdWinSetOsdRamStartAddr(FOSD_WIN2, columns*FOSD_WIN2);//text_info->osdram
    //clear all OsdRam.
    //FOsdRamClearAll(0x020, 0x0F);
}

void WinStringDisplay(BYTE WinNo,WORD x, WORD y,BYTE * Str)
{
    BYTE len;
    BYTE columns;
    BYTE attr;
	...
	memset(FOsdMsgBuff,0,sizeof(FOsdMsgBuff));
    TWstrcpy(FOsdMsgBuff,Str);
    FOsdWinScreenXY(WinNo, 1280-x,y);//30*winno);
#if 0
...
//	len += 1;	//test
    ...
    //cut end of line
    FOsdWinScreenWH(WinNo, len,1);

    columns = FontOsdInfo.win[WinNo].column;
    attr = 	  FontOsdInfo.win[WinNo].bpp1_attr;
    osdram =  FontOsdInfo.win[WinNo].osdram;

    //print string
    WaitVBlank(1); //pls. finish within one vblank.
//    FOsdRamSetAddrAttr(osdram, attr);	//osdram address, fg|bg, mode:auto
	FOsdRamSetAddr(osdram);
...
[00:313]
*/
//const  BYTE calculated_3BPP_alpha_table[8] = { 1,7,6,4,3,5,2,0 };

//read value from FontEditor and place it here.
//const  WORD consolas22_16x26_3BPP[8] = {
...
/******************
	FOSD ¾µÏñ
******************/
void FOSD_Mirror(void)
{
    TW8836_WritePage(PAGE3_FOSD);
    WriteTW88(REG300, (ReadTW88(REG300)) | 0x04 );
...
//	SpiFlash_Read_FOsd(src_loc, dest_loc, size); 	//modify by Feng
...
* replace FontRam
*/
void ReplaceFontRam(WORD dest_font_index, const FONT_SPI_INFO_t *font, char *sName)
{
    WORD addr;
...
*/
void InitFontRam(WORD dest_font_index, const FONT_SPI_INFO_t *font, char *sName)
{
    BYTE value;
    WORD addr;
...
//	FOsdWinScreenWH(FOSD_MSG_WIN, len, 20);
...
void FOsdIndexMsgPrint(BYTE index)
{
    ...
    */
    ...
    case FOSD_STR1_TW8836:
        TWstrcpy(FOsdMsgBuff,"Intersil TW8836.01");
        break;
...
* print Font Info
*/
static void FontInfo(WORD start, const FONT_SPI_INFO_t *font)
{
    BYTE i;
    WORD *wptr;
...
void DumpFont(void)
{
    ...
    FOsdWinScreenWH(0, (size >=16 ? 16: size), (size >> 4) + (size & 0x0F ? 1 : 0));
    Y = ((size >> 4) + 1)* FontOsdInfo.font.h + 10;
    FOsdWinZoom(0, 1/*0*/,0);
    ...
    FOsdWinScreenWH(1, (size >=16 ? 16: size), (size >> 4) + (size & 0x0F ? 1 : 0));
    Y += ((size >> 4) + 1)* FontOsdInfo.font.h + 10;
    FOsdWinZoom(1, 1/*0*/,0);
    ...
    FOsdWinScreenWH(2, (size >=16 ? 16: size), (size >> 4) + (size & 0x0F ? 1 : 0));
    Y += ((size >> 4) + 1)* FontOsdInfo.font.h + 10;
    FOsdWinZoom(2, 1/*0*/,0);
