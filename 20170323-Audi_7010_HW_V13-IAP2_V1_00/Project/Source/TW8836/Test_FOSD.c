...
//	BYTE start_offset = 0xA0;
    ...
    FOsdWinScreenXY(fosd->win, 800,0);		//0x20, 0x10);		//max 512 = 32x16
    FOsdWinScreenWH(fosd->win, columns, lines);		//0x20, 0x10);		//max 512 = 32x16
    FOsdWinZoom(fosd->win, 1, 0);						//fosd->zoom>>4, fosd->zoom & 0x0F);
	...
	FOsdWinScreenWH(FOSD_MSG_WIN, columns,1);
...
//const FONT_SPI_INFO_t consolas16x26_606C90_test 	= { 0x402000, 0x2080, 16, 26, 0x060, 0x06C, 0x090, 0x0A0 };
...
const WORD consolas16x26_606C90_2BPP_test[4] =
{
    0xF7DE,0x0000,0x5AAB,0xC000
};

const WORD consolas16x26_606C90_3BPP[8] =
{
    0xFFFF,0x0000,0xDEDB,0x9492,0x6B6D,0xB5B6,0x4A49,0x2124
};
const WORD consolas16x26_606C90_4BPP[16] =
{
...
void TestNum1BPP3BPP(void)
{
    ...
    FOsdWinScreenWH(fosd->win, columns, lines);		//0x20, 0x10);		//max 512 = 32x16
    FOsdWinZoom(fosd->win, 0, 0);						//fosd->zoom>>4, fosd->zoom & 0x0F);
    ...
    palette = 32;
    FontOsdSetPaletteColorArray_TEST(palette, (WORD *)consolas16x26_606C90_3BPP, 8);
    //FOsdWinAlphaPixel(fosd->win, palette+0,8); //NOTE:LUT32 do not support Alpha.
    ...
    palette = 48;
    FontOsdSetPaletteColorArray_TEST(palette, (WORD *)consolas16x26_606C90_4BPP, 16);
    FOsdRamSetAddrAttr(4, palette >> 2);  //overwrite
