...
#define SendString(a, b)    Usart2SendString(a, b)     //串口发送函数 a:字符串 b:长度
...
{  
   ...
   {
       CRC_Temp = CRC_Temp^(int)(*pData++) << 8; //    
       for (i=8; i!=0; i--) 
       {
           if (CRC_Temp & 0x8000)   
               CRC_Temp = CRC_Temp << 1 ^ 0x1021;    
           else
               CRC_Temp = CRC_Temp << 1;
       }    
   }
...
int YmodemReceive(char *pRece, int *sReceLen, char *pData, int *sResLen)
{
	...
	u8 AesBufferIn[16]={0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};;
    ...
    case eYM_RECE_DATA:
        ...
        case TRUE:                      //接收正确
			...
			for(i = 0;i < Length;i++)
			{
				...
				for (j = 0; j < 16; j++)
				{
					AesBufferIn[j] = pData[i * 16 + j];
				//	printf("%02x ",AesBufferIn[j]);
				}
				
				
				if(
					aes_decrypt_block(&ctx, AesBufferOut, AesBufferIn)
					!= AES_SUCCESS
					)
				{
				//	printf("aes_encrypt_block error.");
					YmodemSendChar(NAK, &stat, &sErrorCount); 
					break;
				}
				else
				{
				//	printf("\r\n");
					for (j = 0; j < 16; j++)
					{
						pData[i * 16 + j] = AesBufferOut[j];
				//		printf("%02x ",AesBufferOut[j]);
					}
    ...
    case eYM_END:
        ...
        break;
	case eYM_SUCCESS:
		YmodemSendChar(ACK, &stat, &sErrorCount);
		cRes = YM_EXIT;
        stat = eYM_INIT;
	//	FLASH_Unlock();
		#if updatamode == 1 
		...
		#else
		if(FLASH_COMPLETE == FLASH_ErasePage(UpdataFlagAddress))	//擦除标志
		#endif
		{	
			printf("\r\nUpdataSuccess!!");
			//SYS_PowerOff();
			//printf("\r\nstartpoweroff");
			Delay_1ms(5000);	//power_main 拉低2S,安卓重启
